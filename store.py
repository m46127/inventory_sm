...
def main():
    ...
    uploaded_file_picking2 = st.file_uploader("2つ目のピッキングリストファイルをアップロードしてください", type=['xlsx', 'xls'], key='picking2')
    uploaded_file_inventory = st.file_uploader("在庫表ファイルをアップロードしてください", type=['xlsx'], key='inventory')

    if uploaded_file_picking1 is not None and uploaded_file_picking2 is not None and uploaded_file_inventory is not None:
        # 2つのピッキングリストの処理と合算
        picking_list_grouped1 = process_picking_list(uploaded_file_picking1)
        picking_list_grouped2 = process_picking_list(uploaded_file_picking2)
        picking_list_combined = pd.concat([picking_list_grouped1, picking_list_grouped2]).groupby('メーカー型番')['注文数量の合計'].sum().reset_index()
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            for sheet in inventory_sheets:
                try:
                    ...
                    # 在庫表とピッキングリストの結合
                    result_df = pd.merge(inventory[['ＪＡＮ']], picking_list_combined, left_on='ＪＡＮ', right_on='メーカー型番', how='left')
